...
const index = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    window.location.href = `/search/${searchAnime}?order_by=${
      orderBy || "recent"
    }`;
  };
  const handleOrder = (e) => {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={`Search your favorite anime ${detailsWatch.title} with subtitles in Indonesian.`}
        />
        <meta name="robots" content="index, follow" />
        <meta
          property="og:title"
          content={`Kazuna Live - Search Anime ${detailsWatch.title}`}
        />
        <meta
      ...
      <div className="w-[90%] m-auto mt-5">
        <div className="">
          <div className="flex  justify-between gap-3 items-center ">
            <form action="" onSubmit={handleSubmit}>
              <input
                type="search"
                value={searchAnime}
                className="w-full md:w-96 px-4 py-2 rounded-md bg-[#23242b] flex outline-none text-slate-200 "
            ...
            <div
              className="boxFilter bg-[#23242b] md:w-full px-4 py-3 rounded-md hover:bg-[#353641]"
          ...
          <div
            className={`showModalFilter   duration-400  ${
              showFilter ? "flex flex-col gap-2 mt-4 px-1 " : "hidden"
            }`}
          >
            <div className="grid grid-cols-2 items-center">
              <h1 className="text-white uppercase text-lg font-karla font-light">
                Order by
              </h1>
              <select
                name="genre"
                id=""
                className="px-3 py-2 rounded-md bg-[#23242b] text-white w-full appearance-none"
          ...
          {loading
            ? arr.map((ress, i) => (
                <div className="flex flex-col gap-3">
                  <a
                    href={``}
                    className="cardItem h-[150px] bg-[#ddd] rounded-md w-full animate-pulse"
                    key={i}
                  ></a>
            ...
            : animeSearch.data &&
              animeSearch.data.map((anime, i) => (
                <a
                  href={`/details${anime.animeId}`}
                  className="cardItem"
                  key={i}
                >
                  <div className="images relative">
                    ...
                    />
                    <span className="bg-[#e94513] font-medium absolute bottom-0 text-sm px-2 py-[2px] text-white rounded-[4px] block">
                      TV
                    </span>
                  ...
                  <div className="title">
                    <h1 className="text-white font-karla font-light mt-1">
